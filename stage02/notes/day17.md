...
## pip 安装
- 格式1：python -m pip install+工具名（安装到环境变量下设置的路径）（+ -i + 镜像地址，从镜像地址安装）
- 格式2：路径 + -m pip install+加工具名（指定路径安装）
## matplotlib
- 示例1：
```
...
- 注意：常数求导后直接变成0，积分虽然不杀常数，但是无法补回一系列变化后的位置，因为缺乏一个常数，无法补回常数，所以将速度积分后，得到的不一定是位置，但是图像变化趋势和位置的一模一样。
