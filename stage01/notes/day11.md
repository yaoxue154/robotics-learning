...
## self
- 沿用示例
- 有```person1=Student("Bob",100)```,就会自动调用init，此时self就是person1，person1.name=Bob，person1.grade=100
- 调用函数时，都是self.def(函数名称如```grade_system.find()```)可以调用，不是find（name）了,不过find(name)没有失效，还是可以使用的。self，有自己的名字，可以用grade_system=Grade()来命名
...
- class = 数据的形状 + 操作数据的函数，捆在一起，因为__init__下有数据的模样，就像初始化一样
